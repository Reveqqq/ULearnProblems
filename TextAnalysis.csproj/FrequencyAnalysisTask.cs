...
namespace TextAnalysis
{
    static class FrequencyAnalysisTask
    {
        public static Dictionary<string, string> GetMostFrequentNextWords(List<List<string>> text)
            => GetNGrams(text, 2).Concat(GetNGrams(text, 3)).ToDictionary(k => k.Key, v => v.Value);

        public static Dictionary<string, string> GetNGrams(List<List<string>> text, int n)
            => text.SelectMany(sentence => GetNGramsInSentence(sentence, n)).GroupBy(key => key.Item1, value => value.Item2)
